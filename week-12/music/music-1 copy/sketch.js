...
let size, size2;
let min;
let bass, treble, mid, lowMid, highMid, sprinkles, line;
let bassHist = [];
let trebleHist = [];
let sprinHist = [];
let midHist = [];
...
x = map(max(min, sprinkles), min, 255, width/3, width);
y = map(max(min, sprinHist[10]), min, 255, height/4, height);
ang = map(mid, 0, 255, 0, 360);
...
x = map(max(min, line), min, 255, width/4, width - width/4);
y = map(max(min, lineHist[10]), min, 255, height/3, 0);
ang = map(line, 0, 255, 0, 360);

translate(x, (y + shift)%height/2);

size =  map(line, 0, 255, 0, 40);
rotate(ang);
fill(255, line, lineHist[10]);
